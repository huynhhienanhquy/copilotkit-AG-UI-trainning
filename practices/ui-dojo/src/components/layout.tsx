...
const SIDEBAR: SidebarEntry[] = [
  {
    id: "practice", title: "Practice", icon: Sparkles,
    items: [{ id: "ghibli-practice", title: "Ghibli Workspace", url: "/practice/ghibli",
      description: "Ghibli agent with saved conversations, workspace tools, documents and watchlist",
      explanation: "A complete practice workspace: control theme and sidebar, search and manage conversations, extract attached documents, and keep a Ghibli watchlist. Messages and data are persisted by Mastra and LibSQL." }],
  },
  {
    id: "ai-sdk",
    title: "AI SDK",
...
function ConceptGroup({
  ...
  return (
    <SidebarMenuSubItem>
      <div className="text-sidebar-foreground/70 flex h-7 items-center px-2 text-sm font-medium">
        {concept}
      </div>
      <SidebarMenuSub>
        {items.map((item) => (
...
export default function Page({ children }: { children: React.ReactNode }) {
  ...
  function findEntry(item: SidebarItemEntry): boolean {
    return item.url === fullUrl || item.url === "/practice/ghibli" && location.pathname.startsWith("/practice/ghibli/chat/");
  }
  ...
  const pageTitle = pageTitleEntry?.concept
    ...
    : (pageTitleText ?? "UI Frameworks Showcase");
  const pageDescription = pageTitleEntry?.description;
  const pageExplanation = pageTitleEntry?.explanation;
  const docsUrl = pageTitleEntry?.docsUrl;

  useEffect(() => {
    document.title = pageTitleEntry
      ? `${pageTitle} — Mastra UI Dojo`
      : "Mastra: UI Frameworks Showcase";
  }, [pageTitle, pageTitleEntry]);
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        <header className="flex shrink-0 flex-col items-start gap-2 py-3 lg:flex-row lg:items-center">
                ...
                <Dialog>
                  <DialogTrigger asChild>
                    ...
                    >
                      ...
                      <span className="sr-only">About this example</span>
                    </Button>
                  </DialogTrigger>
                  <DialogContent>
                    <DialogHeader>
          ...
          {pageDescription ? (
            <div className="text-muted-foreground px-4 text-sm lg:px-0">
              {pageDescription}
            </div>
          ) : null}
        </header>
